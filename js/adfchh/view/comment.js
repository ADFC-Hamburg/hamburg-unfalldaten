define('adfchh/view/comment', [
    ...
    function clearErr() {
        $('#comments').find('.help-block').each(function () {
            $(this).remove();
        });
        $('#comments').find('.form-control-feedback').each(function () {
            $(this).remove();
        });
        $('#comments').find('.has-error').each(function () {
    ...
    function addComment() {
        ...
        $.post( 'api/comment.php/new', JSON.stringify(data) ).done( function (d) {
            ...
            if (d.status==1) {
                ...
                $('#newcommentmsg').removeClass().
  addClass('alert alert-success')
  .text('Vielen Dank, der Kommentar wurde gespeichert. Wir melden'+
        ' uns bei Ihnen per E-Mail sobald dieser angezeigt wird.')
  .show();
            } else if (d.status==2) {
                $('#comment-send-btn').button('reset');
                $('#newcomment').show();
...
/*     for (var i = 0; i < d.val.keys().length; i++) {
  ...
  console.log(key);
            ...
            $('#newcommentmsg').removeClass().addClass('alert alert-danger')
     .text('Leider gab es beim Speichern einen unvorhergesehen Fehler.'+
    ' Bitte informieren Sie den Admin unter: adfc2015@sven.anders.hamburg').show();
        });
    }
    ...
    function loadComments( lfnr ) {
     ...
     // FIXME 
                ...
                if (comments.length===0) {
                    container.append($('<div>').addClass('alert alert-success')
     .text('Wir haben leider noch keine Kommentare zur Unfallstelle erhalten.'+
           ' Schreibe den ersten Kommentar!'));
                    ...
                    for (var i = 0; i < comments.length; i++) {
                        ...
                        container.append($('<div id="comment-from">')
         .text('Von '+comment.creator+' am '+convertDate(comment.created.date)+'.'));
